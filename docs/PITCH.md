...
Hero case: one hour of real autonomous-car driving, ROS1 Noetic, 375 gigabytes. Operator told us the GPS failed under a tunnel. Tool ran in 35 seconds. Rover receiver kept a 3D fix with 29 satellites the entire session — no tunnel dropout was physically possible. Real root cause: the moving-base antenna's RTCM correction stream to the rover was broken before the car left the lot. Claude returned a scoped patch — specific RTCM3 message IDs to enable, specific UART link to verify. Twenty-two cents per analysis.
